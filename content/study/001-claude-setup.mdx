...
훅 3종은 stdin JSON 샘플로 단위 검증했고(23 케이스: 경로 순회·접두 충돌·UNC·깨진 feature.json·오래된 리뷰 등 리뷰에서 발견된 우회를 회귀 테스트로 추가), `002-smoke`로 specify → plan → tasks → 승인 리뷰 → SDD → converge → E2E → finish → finishing → archive 사이클을 1회 실주행했다. 세부 결과는 `specs/001-claude-setup/report.md`의 Validation 절에 있다.
