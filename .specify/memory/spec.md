...
[Source: specs/001-claude-setup/spec.md -> User Story 1]
...
[Source: specs/001-claude-setup/spec.md -> User Story 2]
...
[Source: specs/001-claude-setup/spec.md -> User Story 3]
...
[Source: specs/001-claude-setup/spec.md -> User Story 4]
...
[Source: specs/001-claude-setup/spec.md -> User Story 5]
...
[Source: specs/001-claude-setup/spec.md -> User Story 6]
...
[Source: specs/001-claude-setup/spec.md -> User Story 7]
...
- 훅 스크립트의 **입력 파싱** 오류는 exit 0·무출력(fail-open)으로 작업을 막지 않는다. finish-gate는 스킬 매칭 이후의 게이트 로직에서 발생한 예외·git 부재·깨진 feature.json을 모두 deny(사유 포함)로 처리한다(fail-closed, 리뷰 반영 2026-08-26). approval-review는 전 구간 fail-open; tester-write-guard는 경로 판정 단계의 예외를 deny로 처리한다. [Source: specs/001-claude-setup/spec.md -> "훅 스크립트의 입력 파싱 오류는"]
- `specify upgrade`가 `.claude/skills/speckit-*`를 덮어써도 name-only 설정은 `settings.json`의 `skillOverrides`에 있으므로 유지된다. 업그레이드 후에는 `specify check`, `tests/run-all.ps1`과 `docs/runbooks/spec-kit-upgrade.md`의 재검증 목록을 수행한다. [Source: specs/001-claude-setup/spec.md -> "specify upgrade가 .claude/skills/speckit-*를 덮어써도"]
- 두 superpowers 플러그인이 다시 동시에 켜지면 SessionStart 훅이 이중 주입된다 → CLAUDE.md의 전제조건 절에 명시. [Source: specs/001-claude-setup/spec.md -> "두 superpowers 플러그인이 다시 동시에 켜지면"]
...
**설치·기반**
- **FR-001**: 저장소는 git으로 초기화되고 기본 브랜치는 `main`이며 `.gitattributes`(`* text=auto eol=lf`)를 둔다. GitHub 원격(private)을 `gh repo create`로 만들고 `main`과 feature 브랜치를 push한다. 작업은 `NNN-slug` 브랜치(또는 `.worktrees/NNN-slug`)에서만 한다. [Source: specs/001-claude-setup/spec.md -> FR-001]
- **FR-002**: Spec Kit은 `specify init --here --integration claude --script ps`로 설치하고, 번들 확장 `git`·`agent-context`와 커뮤니티 확장 `archive`를 설치한다(아카이브 URL 검토 후 `--from`; adrkit은 spec-kit <0.16 게이트로 제외, ADR은 MADR 수작성). [Source: specs/001-claude-setup/spec.md -> FR-002]
- **FR-003**: `git` 확장 설정은 `branch_numbering: sequential`, 기본 템플릿 `{number}-{slug}`, `auto_commit.default: false`, `commit_style: conventional`이다. [Source: specs/001-claude-setup/spec.md -> FR-003]
- **FR-004**: 사용자 레벨 `~/.claude/settings.json`에서 `superpowers@claude-plugins-official`를 비활성화하고 `superpowers@superpowers-dev`(5.1.0)만 유지한다. [Source: specs/001-claude-setup/spec.md -> FR-004]
- **FR-005**: Spec Kit이 생성한 `speckit-*` 스킬은 `.claude/settings.json`의 `skillOverrides`를 `"name-only"`로 두어 설명을 숨기고 자동 트리거를 막되, 스킬 간 연쇄 호출(`speckit-specify` → `speckit-git-feature`, `approval-review` → `speckit-analyze`)과 컨트롤러의 명시 호출은 허용한다. `"user-invocable-only"`는 모델의 모든 Skill 호출을 차단해 연쇄를 깨므로 쓰지 않는다(2026-08-26 확인). 생성된 SKILL.md는 수정하지 않는다(업그레이드 내구성). [Source: specs/001-claude-setup/spec.md -> FR-005]

**문서·경로**
- **FR-006**: `CLAUDE.md`는 200줄 이하, 첫 줄에 `@AGENTS.md`를 import하며, Spec Kit `agent-context` 확장의 `<!-- SPECKIT START/END -->` 관리 블록을 포함한다. [Source: specs/001-claude-setup/spec.md -> FR-006]
...
**constitution**
- **FR-014**: constitution은 최소 6개 원칙을 갖는다 — Spec-First, Test-First(NON-NEGOTIABLE), Tenant Boundary(모든 데이터 소유·격리 경계를 spec에 명시), Observability-Ready(로그·메트릭·롤백 경로를 plan에 명시), Simplicity(YAGNI), Learning-in-Public(feature마다 학습 노트). Governance 절에 개정 규칙(SemVer)을 둔다. [Source: specs/001-claude-setup/spec.md -> FR-014]
- **FR-015**: `.specify/templates/overrides/tasks-template.md`는 테스트 task를 필수로 만들고 User Story마다 E2E task를 포함시킨다. [Source: specs/001-claude-setup/spec.md -> FR-015]

**에이전트·스킬·훅**
- **FR-016**: `.claude/agents/tester.md`는 tools `Read, Grep, Glob, Bash, Edit, Write`를 가지며, frontmatter `hooks`로 `PreToolUse(Edit|Write)`에 `tester-write-guard.ps1`을 걸어 테스트 경로 외 쓰기를 deny한다. spec의 User Story별 PASS/FAIL/SKIP과 재현 절차를 보고한다. [Source: specs/001-claude-setup/spec.md -> FR-016]
- **FR-017**: `.claude/skills/approval-review/`는 `boundaries/*.md`(security, tenant-data, operability, trends, spec-consistency)마다 서브에이전트 1개를 병렬 디스패치하고, spec-consistency 경계는 `/speckit-analyze` 결과와 checklist 상태를 사용한다. 결과는 `reviews/YYYY-MM-DD-approval.md`에 기록하고 사용자 확정 후 `spec.md` Status를 `Approved`로 바꾼다. [Source: specs/001-claude-setup/spec.md -> FR-017]
- **FR-018**: `.claude/skills/finish/`는 순서대로 ① `report.md` 생성 ② `content/study/NNN-slug.mdx` 초안 생성 ③ `CHANGELOG.md` Unreleased 갱신 ④ 필요 시 `/speckit-adrkit-draft` 안내 ⑤ `docs/kr` 미러 동기화(best-effort, 미완료 시 `translation-pending` 표시, 마감 비차단) ⑥ `boundaries/*.md`(report-vs-diff, e2e-evidence, study-contract, decisions)별 서브에이전트 병렬 리뷰 → `reviews/YYYY-MM-DD-finish.md`(`Status: Approved | Issues`)를 수행한다. Issues면 수정 후 ⑥을 재실행한다. [Source: specs/001-claude-setup/spec.md -> FR-018]
- **FR-019**: 훅은 세 개다. `approval-review.ps1`(UserPromptSubmit: 승인 키워드 `승인|approve|approved|LGTM|진행해` 감지 시 `/approval-review` 실행 지시를 `systemMessage`로 출력), `finish-gate.ps1`(PreToolUse matcher `Skill`: `tool_input.skill`이 `finishing-a-development-branch`를 포함하면 활성 feature를 D12 순서(env → 브랜치명 → feature.json)로 해석하고, 해석 실패·불일치 또는 `reviews/*-finish.md`의 `Status: Approved`·`report.md`·`content/study/<basename>*.mdx` 중 하나라도 없으면 `permissionDecision: deny`), `tester-write-guard.ps1`(PreToolUse Edit|Write: 경로 화이트리스트 외 deny). 입력 파싱 오류 시 exit 0·무출력; finish-gate·tester-write-guard의 판정 단계 예외는 deny(사유 포함). [Source: specs/001-claude-setup/spec.md -> FR-019]
- **FR-020**: `.claude/settings.json`은 `permissions.deny`에 `Bash(rm -rf *)`, `Bash(git reset --hard*)`, `Bash(git push --force*)`, `Bash(git push -f*)`, `Bash(git clean -fd*)`, `Bash(docker system prune*)`를 두고 훅 2종(approval-review, finish-gate)을 등록한다. 훅 명령은 `pwsh -NoProfile -ExecutionPolicy Bypass -File "${CLAUDE_PROJECT_DIR}/.claude/hooks/<name>.ps1"`(PowerShell 7; 플레이스홀더로 cwd 변경에 견딤)이며, deny 규칙은 `Bash(...)`와 `PowerShell(...)` 양쪽에 둔다(도구별 별도 네임스페이스). [Source: specs/001-claude-setup/spec.md -> FR-020]
- **FR-021**: `.claude/rules/`는 `specs.md`(paths: `specs/**`), `docs.md`(paths: `docs/**`), `content.md`(paths: `content/**`) 세 파일이며 각각 해당 경로의 형식·계약만 담는다. [Source: specs/001-claude-setup/spec.md -> FR-021]

**콘텐츠 계약**
- **FR-022**: `content/study/*.mdx` frontmatter는 `title`(string), `description`(string), `pubDate`(ISO date), `updatedDate?`, `tags`(string[]), `series?`(string), `seriesOrder?`(int), `draft`(bool, 생성 시 true), `change`(string, `NNN-slug`), `sources`({title, url|path}[])를 가진다. 파일명은 ASCII kebab-case(`NNN-slug.mdx`)다. [Source: specs/001-claude-setup/spec.md -> FR-022]
- **FR-023**: 학습 노트 본문은 `## 문제`, `## 배운 개념`, `## 선택과 대안`, `## 결과와 검증`, `## 다음 학습` 5개 절을 가진다. [Source: specs/001-claude-setup/spec.md -> FR-023]
...
- **Review**: `reviews/YYYY-MM-DD-{approval|finish}.md`. 경계(boundary)별 절 + 종합 판정. 경계 정의는 스킬의 `boundaries/*.md`. [Source: specs/001-claude-setup/spec.md -> "Review"]
- **Report**: `report.md`. Summary / Changes Made / Validation / Next 4절. [Source: specs/001-claude-setup/spec.md -> "Report"]
- **StudyNote**: `content/study/NNN-slug.mdx`. FR-022 계약. 사이트(SP-1)가 소비. [Source: specs/001-claude-setup/spec.md -> "StudyNote"]
- **Decision**: `docs/decisions/NNNN-*.md`. MADR 4.0 minimal. adrkit이 plan 검토에 사용. [Source: specs/001-claude-setup/spec.md -> "Decision"]
- **Constitution**: `.specify/memory/constitution.md`. 원칙·게이트·거버넌스. plan의 Constitution Check가 참조. [Source: specs/001-claude-setup/spec.md -> "Constitution"]
...
- **SC-001**: `specify check`와 `tests/run-all.ps1`이 통과하고 `.claude/skills/speckit-*` 10개 + 확장 스킬이 모두 `settings.json` `skillOverrides`에 `name-only`로 등록되어 있다(생성 파일 무수정). [Source: specs/001-claude-setup/spec.md -> SC-001]
- **SC-002**: 훅 3종이 샘플 stdin JSON으로 단위 검증된다 — approval 키워드 → systemMessage 출력, 비키워드 → 무출력; finish-gate 산출물 없음 → deny / 있음 → 통과 / feature.json 없음 + 브랜치 `NNN-slug` → 브랜치로 해석 / 해석 불가·불일치 → deny; tester-write-guard `tests/e2e/a.test.ts` → allow, `src/a.ts` → deny. [Source: specs/001-claude-setup/spec.md -> SC-002]
- **SC-003**: `002-smoke`가 US1의 시나리오 1~5를 모두 통과하고, `specs/002-smoke/`에 spec·plan·tasks·reviews(approval, finish)·report가, `content/study/002-smoke.mdx`와 `CHANGELOG.md` Unreleased 항목이 존재하며 `.specify/memory/spec.md`에 통합된다. [Source: specs/001-claude-setup/spec.md -> SC-003]
- **SC-004**: `CLAUDE.md`가 200줄 이하이고, `/hooks`·`/agents`·`/skills`에 훅 2종·tester·프로젝트 스킬 2종이 등록되어 보인다. [Source: specs/001-claude-setup/spec.md -> SC-004]
- **SC-005**: `docs/kr/`가 프로젝트 작성 에이전트 파일 전부(CLAUDE, AGENTS, constitution, rules 3, tester, 스킬 2)에 대해 `_kr` 미러를 갖는다(커버리지 100%). [Source: specs/001-claude-setup/spec.md -> SC-005]
- **SC-006**: `docs/decisions/0000`, `0001`이 MADR 형식이고 `specs/README.md`가 001·002를 올바른 상태로 나열한다. [Source: specs/001-claude-setup/spec.md -> SC-006]
...
SP-0(Claude Code 기반 셋팅, `specs/001-claude-setup`)에서 확정된 도구 분담·디렉터리 구조·워크플로우·컴포넌트·데이터 흐름·에러 처리·검증 전략·확장 경로·로드맵·리스크다. 결정 표와 본문의 "실행 기록" 주석(예: adrkit 미설치, skillOverrides name-only)은 원문 그대로다.
...
첫 서브 프로젝트 **SP-0: Claude Code 기반 셋팅**의 범위와 결정은 `specs/001-claude-setup`에서 확정되었다.

SP-0 설계는 superpowers `brainstorming` 스킬로 도출되었다(Spec Kit 설치 전에 작성되어 `spec-template.md`의 필수 절을 따르되 `## Design` 절을 덧붙인 형식). 조사 근거: `specs/001-claude-setup/research/2026-08-26-research-summary.md`. 002번 feature부터는 `/speckit-specify`가 spec을 생성한다. [Source: specs/001-claude-setup/spec.md -> "이 문서는 superpowers brainstorming 스킬로 도출한 설계 스펙이다"]

SP-0 브레인스토밍에서 확정된 결정:
...
| D13 | 생성 스킬 통제 | **`settings.json` `skillOverrides: name-only`로 자동 트리거 차단**(SKILL.md 무수정; user-invocable-only는 연쇄 호출을 막아 불가) | Spec Kit 업그레이드 내구성 |
...
[Source: specs/001-claude-setup/spec.md -> "배경과 결정 요약"]
...
[Source: specs/001-claude-setup/spec.md -> "### 1. 아키텍처 — 역할 분담"]
...
[Source: specs/001-claude-setup/spec.md -> "### 2. 디렉터리 구조 (SP-0 완료 시점)"]
...
[Source: specs/001-claude-setup/spec.md -> "### 3. 워크플로우 — 전체 킷 배치"]
...
[Source: specs/001-claude-setup/spec.md -> "### 4. 컴포넌트 명세"]
...
[Source: specs/001-claude-setup/spec.md -> "### 5. 데이터 흐름"]

### 6. 에러 처리

- 훅: 입력 JSON 파싱 실패 → exit 0, 무출력(fail-open). finish-gate는 게이트 로직 예외를 deny + 사유로 처리하고(fail-closed), 게이트 불충족도 deny + 사유.
...
- Spec Kit 업그레이드 후: `docs/runbooks/spec-kit-upgrade.md` 절차(`specify check` + `tests/run-all.ps1` + 레지스터의 재검증 명령). `skillOverrides`는 settings에 있어 영향 없음.

[Source: specs/001-claude-setup/spec.md -> "### 6. 에러 처리"]

### 7. 테스트·검증 전략

- 단위: 훅 3종 × 시나리오(SC-002)를 PowerShell에서 stdin JSON으로 실행해 stdout 검증.
- 통합: Claude Code에서 `/hooks`, `/agents`, `/skills` 등록 확인, `specify check`.
...
[Source: specs/001-claude-setup/spec.md -> "### 7. 테스트·검증 전략"]
...
[Source: specs/001-claude-setup/spec.md -> "### 8. 확장 경로 (SP-1 이후)"]

### 9. 서브 프로젝트 분해 (로드맵)

| SP | 내용 | 진입 |
|----|------|------|
| SP-0 | Claude Code 기반 셋팅 (`specs/001-claude-setup`) | 부트스트랩 예외 |
...
[Source: specs/001-claude-setup/spec.md -> "### 9. 서브 프로젝트 분해 (로드맵)"]
