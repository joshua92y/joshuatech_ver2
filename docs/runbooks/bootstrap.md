...
- [ ] **OCI 읽기 사용자 `svc-verify` 생성 + 그룹 `jt-verify` 가입** — 위 서비스 사용자 항목과 같은 콘솔(Identity & Security → Domains → (기본 도메인) → Users)에서 사용자 생성, Groups에서 `jt-verify` 생성 후 가입. 그룹에 붙는 정책 선언은 T010 OpenTofu 몫(스코프는 표 ③). 진행(2026-09-03): 사용자 생성·그룹 가입 완료 — **이름 예외(사용자 결정 2026-09-03): 그룹 실명은 `joshuatech-verify`**(T010 정책·문서의 `jt-verify` 표기는 이 이름으로 읽는다). ⚠️ 미결: 생성된 Customer Secret Key `svc-verify-screat-key`는 표 ③의 "상주 자격 없음" 원칙 위반 — **콘솔에서 삭제** 후 이 항목 체크(svc-verify는 세션 인증 전용).
