...
- [ ] **Grafana Cloud Alloy 전송용 Access policy 토큰** — Grafana Cloud 포털(grafana.com) → Security → Access Policies → Create access policy(realm: 스택 `joshuatech`, scopes: `metrics:write`·`logs:write`·`traces:write`) → Add token. 값은 비밀번호 관리자에만 기록(토큰 표 참고). 진행: 정책 `joshuatech-grafana-alloy` 생성 완료(2026-09-02, scopes 3종 write 확인) — **Add token + 비밀번호 관리자 보관 후 체크**.
