# 문서 인덱스

| 문서 | 내용 |
|---|---|
| [CLAUDE.md](../CLAUDE.md) · [kr](kr/CLAUDE_kr.md) | Claude Code 운영 규칙(도구 경계·라이프사이클·feature 해석·훅) |
| [AGENTS.md](../AGENTS.md) · [kr](kr/AGENTS_kr.md) | 도구 중립 프로젝트 브리프 |
| [constitution](../.specify/memory/constitution.md) · [kr](kr/constitution_kr.md) | 개발 헌법 1.0.0(6원칙·게이트·거버넌스) |
| [decisions/](decisions/) | ADR(MADR 4.0 minimal) |
| [decisions/0002](decisions/0002-deployment-principles.md) | ADR 0002 배포 원칙 — L1 원칙 7(불변 digest·Git 정본·pull CD)·부록 D10 Dragonfly |
| [decisions/0003](decisions/0003-runtime-track.md) | ADR 0003 런타임 트랙 — 웹 Workers + API OCI K3s 2노드(A platform/B data) + Argo CD·부록 D14 관측 |
| [decisions/0004](decisions/0004-web-framework.md) | ADR 0004 웹 프레임워크 — Next.js 16.3 + OpenNext lean(프리렌더 + BFF·번들 예산)·부록 D19 콘텐츠 정본 |
| [decisions/0005](decisions/0005-backend-framework-policy.md) | ADR 0005 백엔드 프레임워크 정책 — Django+Ninja 기본·FastAPI 수치 트리거 예외·부록 D18 pod 목록 |
| [decisions/0006](decisions/0006-identity-and-authz.md) | ADR 0006 신원·인가 — Authentik 단일 IdP·impersonation 교환 기본(VD-1)·거부 목록·OpenFGA |
| [decisions/0007](decisions/0007-data-ownership-and-tenancy.md) | ADR 0007 데이터 소유·테넌시 — CNPG DB per pod·테이블 등급 A/B·FORCE RLS·멤버십 정본 TenantMembership |
| [decisions/0008](decisions/0008-event-backbone.md) | ADR 0008 이벤트 백본 — Kafka KRaft(Strimzi)·폴링 outbox 릴레이·CloudEvents JSON + JSON Schema·DLQ·purge |
| [decisions/0009](decisions/0009-search.md) | ADR 0009 검색 — ES 1노드 + Kibana + Nori·ECK·FastAPI search pod(SP-3 구현, 노드 B ≈ 3 GiB 자리)·pg_bigm 이월 |
| [decisions/0010](decisions/0010-secrets.md) | ADR 0010 시크릿 — Vault Raft + OCI KMS auto-unseal + ESO store 5·recovery 3/2(unseal 불가)·일 1회 Raft 스냅샷 |
| [runbooks/bootstrap.md](runbooks/bootstrap.md) | 플랫폼 부트스트랩 런북 — §0 사용자 수동 준비·토큰 표(§3 T031–T044 실행 기록 · §4 Vault init·시드 · §6 업그레이드 창) |
| [runbooks/vault-unseal.md](runbooks/vault-unseal.md) | Vault seal 런북 — KMS 장애(재시작 = CrashLoop)·break-glass(2.0은 유효 토큰 필수)·Raft 스냅샷 복원(같은 KMS 키 필수)·재초기화 절차·init 전사 안전 블록, 전부 워크스테이션 CLI |
| [runbooks/spec-kit-upgrade.md](runbooks/spec-kit-upgrade.md) | Spec Kit 커스터마이즈 레지스터·업그레이드 절차 |
| [specs/README.md](../specs/README.md) | feature 인덱스(불변 이력) |
| [content/study/](../content/study/) | 학습 노트(learning in public) |
| [kr/](kr/) | 에이전트 파일 한국어 미러(편의용, 정본은 영어) |
