...
- 훅 스크립트의 **입력 파싱** 오류는 exit 0·무출력(fail-open)으로 작업을 막지 않는다. finish-gate는 스킬 매칭 이후의 게이트 로직에서 발생한 예외·git 부재·깨진 feature.json을 모두 deny(사유 포함)로 처리한다(fail-closed, 리뷰 반영 2026-08-26). approval-review·tester-write-guard는 전 구간 fail-open.
...
- **FR-019**: 훅은 세 개다. `approval-review.ps1`(UserPromptSubmit: 승인 키워드 `승인|approve|approved|LGTM|진행해` 감지 시 `/approval-review` 실행 지시를 `systemMessage`로 출력), `finish-gate.ps1`(PreToolUse matcher `Skill`: `tool_input.skill`이 `finishing-a-development-branch`를 포함하면 활성 feature를 D12 순서(env → 브랜치명 → feature.json)로 해석하고, 해석 실패·불일치 또는 `reviews/*-finish.md`의 `Status: Approved`·`report.md`·`content/study/<basename>*.mdx` 중 하나라도 없으면 `permissionDecision: deny`), `tester-write-guard.ps1`(PreToolUse Edit|Write: 경로 화이트리스트 외 deny). 입력 파싱 오류 시 exit 0·무출력; finish-gate의 게이트 로직 예외는 deny(사유 포함).
