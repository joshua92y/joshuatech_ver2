...
| `ALL /api/proxy/<pod>/<path>` | 세션 + CSRF(비-GET) | SP-2용 골격. `<pod>`는 허용 목록(`identity-admin`만 SP-1). 교환된 토큰 + 서비스 토큰으로 `https://<alias>-m2m-<env>.joshuatech.dev/<path>` 호출, 응답 그대로 중계 | 상류 상태 |
...
- 폐기 확인: `GET https://identity-m2m-prod.joshuatech.dev/session/check?sub=<sub>&iat=<iat>&sid=<sid>` → `{ active: boolean }`. 결과를 2초 캐시. `active=false`면 쿠키 삭제 + 401 `session_revoked`.
