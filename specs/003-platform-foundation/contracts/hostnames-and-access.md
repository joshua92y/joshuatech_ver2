...
1. OCI NSG: 노드 A ingress **443만** ← Cloudflare IPv4 대역(`data.cloudflare_ip_ranges`, OpenTofu 주기 실행으로 갱신). 80은 열지 않는다(Always Use HTTPS는 edge, LE는 DNS-01). 노드 B ingress 없음. 클러스터 내부는 NSG 자기참조. 22는 평시 닫힘 — break-glass는 **OCI CLI로 `nsg-cluster`에 운영자 `<ip>/32` 22/tcp 임시 규칙을 추가·제거**하는 절차다(`infra/oci/instances.tf` 머리 주석 5·8단계; 제거 뒤 규칙 수 1이 유일한 증거). OpenTofu 코드에는 두지 않는다 — 하네스 nsg-3·nsg-4가 22 규칙을 fail-closed로 막는다. 읽기 전용 `svc-verify` 프로파일로는 할 수 없다(운영자 프로파일).
