...
- pod API(M2M, Service Auth 전용): `<alias>-m2m-<env>.joshuatech.dev` — env ∈ `prod`·`dev`. alias는 pod 이름의 짧은 형태로 아래 표에 고정한다. m2m Ingress는 경로 허용 목록 `/api`·`/health`·`/healthz`·`/ready`·`/session`·`/sessions`·`/tenants`·`/webhooks`만 노출한다(`/admin/`은 404).
...
| `auth.joshuatech.dev` — 공개 경로 `/application/o/*` · `/if/flow/*` · `/if/user/*` · `/.well-known/*` | Authentik(identity ns, Ingress) | 없음(공개 로그인·OIDC issuer/JWKS) | Authentik 자체 보호(Reputation 정책·MFA). FR-046 예외 2: 클러스터 내부 OIDC discovery/JWKS는 이 공개 호스트를 쓴다(Cloudflare proxied, Access 없음) |
| `auth.joshuatech.dev/if/admin`, `/api/v3` | Authentik 관리 UI·API | **GitHub IdP**(Access 앱 `auth-admin`, 경로 스코프) | 관리자 그룹 `platform-admin`은 WebAuthn 필수. identity-admin의 Authentik API 호출은 svc DNS(`identity` ns 9000)로만 — 이 Access 앱을 지나지 않는다 |
...
| `ssh.joshuatech.dev`, `k8s.joshuatech.dev` | cloudflared 터널 → 노드 A 22 / K3s 6443 | GitHub IdP + `cloudflared access tcp` | 공개 DNS에는 CNAME만, 포트 개방 없음. 에이전트·tester·CI는 아래 `agent-view` 자격만 |
...
- GitHub IdP 앱(`admin`·`argo`·`vault`·`traefik`·`kibana`·`auth-admin`·`preview`·`ssh`·`k8s`): include = 이메일 `egenauto.dev@gmail.com`(운영자), `session_duration` 명시(24h, cloudflare provider 5.24.0에서 필수), MFA는 GitHub 측.
- Zero Trust Free: 50석. 서비스 토큰은 석을 소비하지 않는다.

## 에이전트 · tester · CI 클러스터 자격

- ServiceAccount `agent-view`(ns `kube-system`) = ClusterRole `view` 집계 + `applications.argoproj.io` get/list + Role(`vault` ns) `pods/portforward`. Secret get은 없다.
...
- 운영자 개인 계정(GitHub·Google) 자격은 어떤 환경변수·파일에도 넣지 않는다. E2E 신원은 Authentik 로컬 사용자 `e2e@joshuatech.dev`(Vault `kv/{env}/e2e`); 소셜 로그인은 운영자가 브라우저로 1회 수동 검증.
...
3. 앱 계층: pod가 Access JWT를 검증(Cloudflare를 통과했어도 Access 정책을 지난 요청만) + Bearer의 `act.sub == web-bff`.

## BFF → pod 헤더

```
Authorization: Bearer <exchanged access token, aud=<pod>, act.sub=web-bff>
...
v1 전용 레코드 `api`·`mainapi`는 US8에서 제거. `admin`·`traefik`은 v2가 재사용하므로 OpenTofu `import` 블록으로 가져온 뒤 값을 노드 A reserved IP로 교체한다(T010) — Cloudflare에 같은 이름을 새로 만들면 충돌. provider 핀 `cloudflare ~> 5.24.0` + `.terraform.lock.hcl` 커밋.
