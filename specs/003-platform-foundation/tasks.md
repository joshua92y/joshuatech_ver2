...
- [ ] T075 [P] [US4] `tests/platform/identity.tests.ps1`: web-bff 토큰(테스트용 ROPC 불가 → Playwright가 저장한 토큰 파일 사용)으로 RFC 8693 교환 → `aud=identity-admin`·`tenant_id`·`exp-iat=300`; `audience=portfolio-core` → `invalid_target`; `fga query check user:<sub> member tenant:<id>` → allowed; `admin.` 헤더 없이 403; Argo `--sso` 로그인 — FAIL 확인
