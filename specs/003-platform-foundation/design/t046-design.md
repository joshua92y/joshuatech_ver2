...
| F5 | 차트 기본값 `watchGlobally: true` — values 키 오타(`watchGlobaly` 등)는 **조용히 전역 모드 + ClusterRole**로 되돌아간다(values 스키마에 `additionalProperties: false` 없음). 렌더 결과 검사가 유일한 방어선이다 | T045 G1 교훈과 같은 종류 |
