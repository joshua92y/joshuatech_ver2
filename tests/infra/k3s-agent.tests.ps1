...
#          cfg-4 node-label 정확히 ["role=data"]  cfg-5 write_if_changed ... 600  cfg-6 렌더된 최상위 키가 허용 집합(3개)과 정확히 같다
#          cfg-7 main 순서 check_server_reachable < check_token_file < render_config < install_k3s < ensure_service < wait_node_registered
...
Test-Group 'cfg' {
    ...
    $allowed = @('server', 'token-file', 'node-label')
    $keys = @($cfgKeys.Keys)
