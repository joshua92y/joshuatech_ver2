...
function Detail($r) { "$($r.out) [code=$($r.code)]" }
...
function Remove-Fixtures {
    foreach ($f in $script:fixtures) { Remove-Item $f -Recurse -Force -ErrorAction SilentlyContinue }
    $script:fixtures = @()
}
...
try {
    ...
    Assert 'approval: keyword -> systemMessage mentions /approval-review' ($r.code -eq 0 -and $r.out -match 'approval-review') (Detail $r)
    $r = Invoke-Hook 'approval-review.ps1' @{ hook_event_name = 'UserPromptSubmit'; prompt = '오늘 날씨 어때'; cwd = $repo } $repo
    Assert 'approval: no keyword -> no output' ($r.code -eq 0 -and [string]::IsNullOrWhiteSpace($r.out)) (Detail $r)
    $r = Invoke-Hook 'approval-review.ps1' @{ hook_event_name = 'UserPromptSubmit'; prompt = 'lgtm'; cwd = $repo } $repo
    Assert 'approval: lgtm (case-insensitive) -> systemMessage' ($r.code -eq 0 -and $r.out -match 'approval-review') (Detail $r)

    # ---------- finish-gate ----------
    Remove-Item Env:SPECIFY_FEATURE_DIRECTORY -ErrorAction SilentlyContinue   # ambient value must not leak into fixtures
    ...
    Assert 'gate: other skill -> no output' ($r.code -eq 0 -and [string]::IsNullOrWhiteSpace($r.out)) (Detail $r)
    $r = Invoke-Hook 'finish-gate.ps1' ($finishing + @{ cwd = $d }) $d
    Assert 'gate: branch resolved, artifacts missing -> deny' ($r.code -eq 0 -and $r.out -match '"permissionDecision":"deny"' -and $r.out -match 'report\.md') (Detail $r)

    $d = New-Fixture '002-smoke' $ready $null
    $r = Invoke-Hook 'finish-gate.ps1' ($finishing + @{ cwd = $d }) $d
    Assert 'gate: branch resolved, artifacts present -> allow (no output)' ($r.code -eq 0 -and [string]::IsNullOrWhiteSpace($r.out)) (Detail $r)

    $d = New-Fixture 'main' $ready $null
    $r = Invoke-Hook 'finish-gate.ps1' ($finishing + @{ cwd = $d }) $d
    Assert 'gate: unresolvable (main, no env, no feature.json) -> deny' ($r.code -eq 0 -and $r.out -match 'could not be resolved') (Detail $r)
    ...
    $r = Invoke-Hook 'finish-gate.ps1' ($finishing + @{ cwd = $d }) $d
    Assert 'gate: branch vs feature.json mismatch -> deny' ($r.code -eq 0 -and $r.out -match 'disagree') (Detail $r)
    ...
    Assert 'guard: src/app.ts -> deny' ($r.code -eq 0 -and $r.out -match '"permissionDecision":"deny"') (Detail $r)
